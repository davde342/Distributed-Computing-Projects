...
## SCRIPT FOR PLOT ALL DATA IN ONE PLOT FOR MAKE LOOK THAT ALL 
## HAVE THE SAME AVG BACKUP AND THE SAME AVG RESTORE
...
for x in range(len(bck)):
    for y in range(len(res)):
        ...
        plt.plot(plot['n_h_pr'], plot['avg_backup'], "-b",  label = "Avg % Backed Up High-P")
        plt.plot(plot['n_h_pr'], plot['avg_restore'], "-r",  label = "Avg % To Restore High-P")
        num_el+=1
plt.xlabel("% Of High Priority Server")
plt.ylabel("%")
