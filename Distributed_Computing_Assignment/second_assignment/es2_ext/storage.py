...
class Backup(Simulation):
    ...
    TOT_BLOCK = 0       # count the number of total blocks inside the simulator
 
    TOT_BLOCK_PRIORITY= 0 #calculate the TOT_BLOCK_NUM
    TOT_BLOCK_BACKUP  = 0  # count the number of backed up blocks belonging to priority peers
    TOT_BLOCK_RESTORE = 0  #  count the number of blocks belonging to priority peers still to restore

    def __init__(self, nodes: List['Node'], high_priority_per, per_restore, per_backup, auto_per):
        super().__init__()  # call the __init__ method of parent class
        
       
        assert per_restore > 0 and per_backup > 0
        assert high_priority_per >= 0 and high_priority_per <= 100

        self.avg_backup = []
        self.avg_restore = []
 
        self.high_priority_per = high_priority_per  #field added for our extension, it represents the percentage of high-priority peers 
        self.nodes = nodes
        self.auto_per = auto_per

        if auto_per == 1:
            Backup.PER_BACKUP = (high_priority_per*0.009608575056692568)             
            Backup.PER_RESTORE =(high_priority_per *0.0005986481332284933)
        else:   
            Backup.PER_BACKUP = per_backup 
            Backup.PER_RESTORE = per_restore 


        n = 0
        #How we assign the priority to the peers:
        if high_priority_per > 0 and high_priority_per <= 100:
            for i in range(int(((len(self.nodes)/100)*high_priority_per))):
                #if we have reached the desired number of high-priority peers we have do
                if n == (int((len(self.nodes)/100)*high_priority_per)):
                    break
                #if we have the client_server configuration, we want to assign the priority just to the client nodes 
                if self.nodes[i].n > 0:
                    self.nodes[i].set_priority()
                    n += 1
    ...
    def schedule_transfer(self, uploader: 'Node', downloader: 'Node', block_id: int, restore: bool):
        ...
        ''' if a low-priority peer needs to back up one of its block, it cannot go on if the counter of high priority blocks
        that are already backed up is less than PER_BACKUP; for the restore process, a low-priority peer cannot go on if the
        counter of the high priority blocks still to restore is more than PER_RESTORE of the total number of high priority blocks'''   

        self.avg_backup.append(Backup.TOT_BLOCK_BACKUP/Backup.TOT_BLOCK)
        self.avg_restore.append(Backup.TOT_BLOCK_RESTORE/Backup.TOT_BLOCK)
      
        
        if Backup.TOT_BLOCK_BACKUP > 0:
            if (restore is True and downloader.priority == 0 and (Backup.TOT_BLOCK_RESTORE/Backup.TOT_BLOCK_PRIORITY > Backup.PER_RESTORE)) or (restore is False and uploader.priority == 0 and (Backup.TOT_BLOCK_BACKUP/Backup.TOT_BLOCK_PRIORITY) < Backup.PER_BACKUP):
                return False
...
class Node:
    ...
    def __post_init__(self):
        ...
        Backup.TOT_BLOCK += self.n

    def set_priority(self):
        Backup.TOT_BLOCK_PRIORITY += self.n 
        self.priority = 1

    def count_block_in_the_network(self):
        count_block = 0
        for (held_local, peer) in zip(self.local_blocks, self.backed_up_blocks):
           '''if the block is saved in the local space or if it is
           saved in one peer it means that the block is in the network'''
           if(held_local == True  or peer is not None):
                count_block += 1
        #If the blocks in the network aren't enough to recover all data, the peer lose some blocks
        if count_block < self.k:
            self.can_recover = False
            self.loss_block = self.n - count_block
        return count_block
    ...
    def schedule_next_upload(self, sim: Backup):
        ...
        for peer, block_id in self.remote_blocks_held.items():
            ...
            if peer.online and peer.current_download is None and not peer.local_blocks[block_id]:
                if sim.schedule_transfer(self, peer, block_id, True):
                    return
        ...
        for peer in sim.nodes:
            ...
            if (peer is not self and peer.online and peer not in remote_owners and peer.current_download is None
                ...
                if sim.schedule_transfer(self, peer, block_id, False):
                    return
    ...
    def schedule_next_download(self, sim: Backup):
        ...
        for block_id, (held_locally, peer) in enumerate(zip(self.local_blocks, self.backed_up_blocks)):
            ...
            if not held_locally and peer is not None and peer.online and peer.current_upload is None:
                if sim.schedule_transfer(peer, self, block_id, True):
        ...
        for peer in sim.nodes:
            ...
            if (peer is not self and peer.online and peer.current_upload is None and peer not in self.remote_blocks_held.keys()
                ...
                if block_id is not None:
                    if sim.schedule_transfer(peer, self, block_id, False):
                        return
...
class Online(NodeEvent):
    ...
    def process(self, sim: Backup):
        ...
        sim.schedule(node.arrival_time + exp_rv(node.average_uptime), Offline(node))
    ...
    def process(self, sim: Backup):
        ...
        for node in [uploader, downloader]:
            sim.log_info(f"{node}: {sum(node.local_blocks)} local blocks, "
                         ...
                         f" totblock {Backup.TOT_BLOCK_PRIORITY}"
                         f" backup block {Backup.TOT_BLOCK_BACKUP}"
                         f" restore block {Backup.TOT_BLOCK_RESTORE}"
                         f" uploader.prio {uploader.priority} "
                         f" downloader.prio {downloader.priority} "
                         )
...
class BlockRestoreComplete(TransferComplete):
    def update_block_state(self):
        ...
        if sum(owner.local_blocks) == owner.k:  # we have exactly k local blocks, we have all of them then
            ...
            logging.info(f"{owner} has successfully restored its data!")
            owner.local_blocks = [True] * owner.n
            if owner.priority:
                Backup.TOT_BLOCK_RESTORE -= (owner.n - owner.k)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("config", help="configuration file")
    parser.add_argument("--max-t", default="200 years")
    parser.add_argument("--seed", help="random seed")
    parser.add_argument("--verbose", action='store_true')
    parser.add_argument("--high_priority_per", help='% of the high priority server inside the system', type=int, default=40)
    parser.add_argument("--per_restore", help='% of the priority block that we need to storage', type=float, default=0.3)
    parser.add_argument("--per_backup", help='% of the priority blocks that we need to backup', type=float, default=0.7)
    parser.add_argument("--csv_file", help=' csv file where we save the code result', default="result")
    parser.add_argument("--csv_perc", help=' csv file where we save the perc result', default="result")
    parser.add_argument("--auto_per", help='if this value is set to 1, we select the % backup and restore automatically (suggest for simulation under 300 years)', default=0, type=int)
    ...
    sim = Backup(nodes,  args.high_priority_per, args.per_restore, args.per_backup, args.auto_per)
    tm = parse_timespan(args.max_t)

    # this only for take the years in and save it on the csv file
    cs_year_time =  tm/31449600    
    ...
    if args.csv_file is not None:
        with open(args.csv_file, 'a', newline = '') as f:
            writer = csv.writer(f)
            num_priotity_loss = 0
            num_no_priority_loss = 0

            for node in sim.nodes:
                if node.priority:
                    num_priotity_loss += node.loss_block
                else:
                    num_no_priority_loss += node.loss_block
                p_loss = 0

            if Backup.TOT_BLOCK_PRIORITY > 0:
                p_loss =  num_priotity_loss/Backup.TOT_BLOCK_PRIORITY

            # duration, perc of high priority block, perc_no_priority_loss, perc_priority_loss 
            writer.writerow([cs_year_time ,args.high_priority_per, num_no_priority_loss/(Backup.TOT_BLOCK-Backup.TOT_BLOCK_PRIORITY), p_loss])
        f.close()

    if args.csv_perc is not None and Backup.TOT_BLOCK_PRIORITY > 0:
        with open(args.csv_perc, 'a', newline = '') as file:
            writer = csv.writer(file)
            # DURATION; BACKUP HIGH PRIORITY PER; PER_BACKUP; PER_RESTORE; AVG BACKUP; AVG_RESTORE
            writer.writerow([cs_year_time, sim.high_priority_per, Backup.PER_BACKUP, Backup.PER_RESTORE, sum(sim.avg_backup)/len(sim.avg_backup), sum(sim.avg_restore)/len(sim.avg_restore)])
