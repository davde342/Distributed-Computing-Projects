...
class Queues(Simulation):
    ...
    def __init__(self, lambd, mu, n, d, max_t, shape):
        ...
        self.gen = weibull_generator(float(shape), 1/self.arrival_rate) 
        self.schedule(self.gen(), Arrival(0))  # schedule the first arrival
    ...
    def schedule_arrival(self, job_id):
        ...
        self.schedule(self.gen(), Arrival(job_id))
...
def main():
    ...
    parser.add_argument('--n', type=int, default=20, help="number of servers")
    ...
    parser.add_argument('--shape', help="select the shape", default=1)
    ...
    sim = Queues(args.lambd, args.mu, args.n, args.d, args.max_t, args.shape)
    sim.run(args.max_t)
    ...
    if args.mu == 1 and args.lambd != 1:
        print(f"Theoretical expectation for random server choice (d = args.d): {1 / (1 - args.lambd)}")
    ...
    with open(args.csv_ql, mode='a', newline='') as file:
        ...
        for key, value in sim.time.items():
            writer.writerow([args.shape, sim.d, sim.lambd, value, key])
        file.close()
